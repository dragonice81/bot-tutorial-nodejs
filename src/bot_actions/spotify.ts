...
export const isSpotifyPlaying = async () => {
...
const getSearchTerm = (message: GroupMeMessage) => {
  const {text} = message;
...
export const play = async (message: GroupMeMessage) => {
  await checkSpotifyApi();
  const song = await search(getSearchTerm(message: GroupMeMessage));
  if (!song) {
