...
const sendPictureOfDarby = async (message: GroupMeMessage) => {
...
export const sendGif = async (message: GroupMeMessage) => {
  ...
  try {
    ...
    if (parsedData && parsedData.data) {
      if (parsedData.data.length) {
        ...
        await sendGif({text: '#random', group_id: message.group_id, name: message.name});
      }
    } else {
      logger.warn(`No gifs for ${message.text}`);
      await sendGif({text: '#random', group_id: message.group_id, name: message.name});
